    ...
    <figure class="function-map" aria-label="Function anatomy map">
      <svg viewBox="28 28 954 366" role="img" aria-labelledby="function-map-title function-map-desc">
        ...
        <desc id="function-map-desc">
          A unified function spec connects invocation, discovery, input, output, reactivity, suggestions, selection,
          presentation, and form metadata.
        </desc>
        <defs>
          <marker
            id="map-arrow"
        ...
        <g class="map-lines">
          {functionMapConnectors.map((connector) => (
            <path
              class={[
                connector.bidirectional ? 'map-line-bidirectional' : '',
                connector.dotted ? 'map-line-dotted' : '',
              ].join(' ')}
        ...
        {functionMapBoxes
          .filter((box) => box.id !== 'ideal-function')
          .map((box) => (
            <g
              class="map-card"
              role="link"
